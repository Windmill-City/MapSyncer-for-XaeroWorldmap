...
public final class BiomeResolver {
                ...
                for (int voxelY = 0; voxelY < 4; voxelY++) {
                    for (int voxelZ = 0; voxelZ < 4; voxelZ++) {
                        for (int voxelX = 0; voxelX < 4; voxelX++) {
                            ...
                            voxels[voxelIndex] = section.getBiomeAt(blockX, blockY, blockZ, false);
                        }
                    }
                }
                grids[idx] = voxels;
    ...
    public static String resolveAtY(
        ...
        for (SectionData s : chunk.sections()) {
            ...
            biome = s.getBiomeAt(lx, fallbackLy, lz, smoothBoundary);
            if (isValidBiome(biome)) {
                return biome;
            }
        }
        ...
        for (SectionData s : chunk.sections()) {
            ...
            for (int tryLy = 0; tryLy <= Constants.CHUNK_SIZE - 1; tryLy++) {
                String candidate = s.getBiomeAt(lx, tryLy, lz, smoothBoundary);
    ...
    private static String resolveBiomeAtAbsoluteY(
        ...
        for (SectionData s : chunk.sections()) {
            if (s.sectionY() != targetSectionY || !s.hasBiomes()) {
                continue;
            }
            biome = s.getBiomeAt(lx, localY, lz, smoothBoundary);
            if (isValidBiome(biome)) {
                return biome;
            }
        }
